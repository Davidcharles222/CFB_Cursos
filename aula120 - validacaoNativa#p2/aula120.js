...
document.querySelector('#btn_validar').addEventListener('click',(evt)=>{
    let estadoValidacao=f_nota.validity//estadoValidação recebe em qual estado de validação esta o campo nota

    if(estadoValidacao.valueMissing){//verifica a propriedade e retorna
        f_nota.setCustomValidity('Poxa, este campo é obrigatório')
    }else if(estadoValidacao.rangeOverflow){//verifica a propriedade e retorna
        f_nota.setCustomValidity('Nossa, que nota alta você digitou')
    }else if(estadoValidacao.rangeUnderflow){//verifica a propriedade e retorna
        f_nota.setCustomValidity('Credo, que nota baixa você digitou')
    }

    f_nota.reportValidity()//valor aparece no campo submit da nota
